...
# Assuming `minute_counts_series` is already a pandas Series with a DatetimeIndex
# Example:
# minute_counts_series = pd.Series(data=[1, 0, 0, ...], index=pd.to_datetime(['2024-10-17 00:20:00', '2024-10-17 00:21:00', ...]))

# Decompose the time series
stl = STL(minute_counts_series, seasonal=61).fit()  # Hourly periodicity assumed (seasonal=61 for minute data)

# Compute seasonality strength
seasonal_var = np.var(stl.seasonal)
total_var = np.var(minute_counts_series)
seasonality_strength = seasonal_var / total_var if total_var != 0 else 0

# Ensure the minute_counts_series is converted to a numeric numpy array
minute_counts_array = minute_counts_series.to_numpy()

# Fourier Transform
fft_result = fft(minute_counts_array - minute_counts_array.mean())
...
print("Autocorrelation Plot for Minute Data:")
autocorrelation_plot(minute_counts_series)
plt.show()

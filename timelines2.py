...
def generate_timelines(df, customer_col, date_col, annotation_cols, title_col=None):
    ...
    for customer_id, group in grouped:
        ...
        group["time_delta_hours"] = time_deltas
        ...
        for i, (date, level, row) in enumerate(zip(dates, levels, group.iterrows())):
            annotation_text = "\n".join(
                [f"{col}: {row[1][col]}" for col in annotation_cols]
            )
            if row[1]["time_delta_hours"] is not None:  # Include time delta if not None
                annotation_text += f"\nTime Delta: {row[1]['time_delta_hours']:.2f} hrs"

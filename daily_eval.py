...
# Assuming `daily_counts_series` is already a pandas Series with a DatetimeIndex
# Example:
# daily_counts_series = pd.Series(data=[1003, 620, 735, ...], index=pd.to_datetime(['2024-10-17', '2024-10-18', '2024-10-19', ...]))

# Decompose the time series
stl = STL(daily_counts_series, seasonal=7).fit()  # Weekly periodicity assumed (seasonal=7)

# Compute seasonality strength
seasonal_var = np.var(stl.seasonal)
total_var = np.var(daily_counts_series)
seasonality_strength = seasonal_var / total_var if total_var != 0 else 0

# Ensure the daily_counts_series is converted to a numeric numpy array
daily_counts_array = daily_counts_series.to_numpy()

# Fourier Transform
fft_result = fft(daily_counts_array - daily_counts_array.mean())
...
print("Autocorrelation Plot for Daily Data:")
autocorrelation_plot(daily_counts_series)
plt.show()

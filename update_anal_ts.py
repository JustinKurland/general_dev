...
def run_time_series_analysis(
    ...
    **kwargs
    ...
    rule : str, optional
        A pandas frequency (e.g., "D" for daily, "W" for weekly, "min" for minute).
        Default is "D".
    ...
    seasonal_period : int, optional
        The assumed seasonal periodicity (e.g., 7 for weekly data, 61 for minute-level data).
        Default is 7.
    ...
    **kwargs:
    ...
    # Handle inconsistency for minute frequency
    if rule == "min":
        time_series.index = np.arange(len(time_series))
    ...
    if plot:
        ...
        plt.xlabel("Index" if rule == "min" else "Date", fontsize=14)
